...
function Harness() {
  const { state, setToken, setStarted, setTaskLoading, setTaskError, clearTaskError, setTaskLoaded, reset } =
    useCandidateSession();
  ...
  return (
    <div>
      ...
      <button
        onClick={() => {
          setTaskLoading();
          setTaskLoaded({ isComplete: false, completedTaskIds: [1], currentTask: null });
        }}
      >
        load-task
      </button>
